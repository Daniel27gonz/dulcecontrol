...
export async function generateStyledQuotationPDF(
  ...
  // Helper: paint page background + decorations
  ...
  y = checkPageBreak(clientSectionHeight + 10);
  ...
  quotation.items.forEach((item, index) => {
    ...
    if (y + rowHeight > pageHeight - bottomMargin) {
      // Draw table border for current page section
      doc.setDrawColor(...primaryColor);
  ...
  // === FOOTER ===
  const footerContentHeight = 45;
  y = checkPageBreak(footerContentHeight);
